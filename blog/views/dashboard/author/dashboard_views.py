    ...
    def post(self, request, *args, **kwargs):
        ...
        if action == self.SAVE_AS_DRAFT:
            ...
            if article_status == Article.PUBLISHED:
                ...
                messages.error(request,
                                "Guardaste el artículo como borrador pero lo seleccionaste "
                                "el estado como 'PUBLICADO'. No puede guardar un "
                                "artículo cuyo estado es 'PUBLICADO' como borrador. "
                                "Cambie el estado a 'BORRADOR' antes que usted "
                                "guardar el artículo como borrador.")
                return render(request, self.template_name, self.context_object)
        ...
        if action == self.PUBLISH:
            ...
            if article_status == Article.DRAFTED:
                ...
                messages.error(request,
                                "Hiciste clic en 'PUBLICAR' para publicar el artículo "
                                " pero seleccionó el estado como 'BORRADOR'."
                                "No puedes Publicar un artículo cuyo estado es "
                                "'BORRADOR'. Cambie el estado a "
                                "'PUBLICADO' antes de poder publicar el "
                                "artículo.")                
                return render(request, self.template_name, self.context_object)
    ...
    def post(self, request, *args, **kwargs):
        ...
        if action == self.SAVE_AS_DRAFT:
            ...
            if not request.user == old_article.author.username:
                messages.error(request=self.request, message="No tienes permiso para actualizar este artículo.")
        ...
        if action == self.PUBLISH:
            ...
            if article_update_form.is_valid():
                ...
                messages.success(self.request, f"Artículo actualizado con éxito.")
...
class ArticleDeleteView(LoginRequiredMixin, View):
    ...
    def get(self, *args, **kwargs):
        ...
        if not self.request.user.username == article.author.username:
            messages.error(request=self.request, message="No tienes permiso para eliminar este artículo.")
            return HttpResponseRedirect(self.request.META.get('HTTP_REFERER', '/'))

        article.deleted = True
        article.save()

        messages.success(request=self.request, message="Artículo eliminado con éxito")
